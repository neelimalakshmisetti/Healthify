...
type MedicalModel = tf.GraphModel;
type MedicalPrediction = { className: string; probability: number }[];
...
const modelCache: { tf?: TfModule; chexnetModel?: MedicalModel; radfmModel?: MedicalModel; evaxModel?: MedicalModel } = {};

const CHEXNET_MODEL_URL = 'https://tfhub.dev/google/imagenet/mobilenet_v2_100_224/classification/5'; // Placeholder: replace with actual CheXNet TF.js model URL
const RADFM_MODEL_URL = 'https://tfhub.dev/google/imagenet/mobilenet_v2_100_224/feature_vector/5'; // Placeholder: replace with actual RadFM TF.js model URL
const EVAX_MODEL_URL = 'https://tfhub.dev/google/imagenet/mobilenet_v2_100_224/feature_vector/5'; // Placeholder: replace with actual EVA-X TF.js model URL

const BODY_PART_KEYWORDS: Record<BodyPartKey, string[]> = {
  chest: ['chest', 'lung', 'thorax', 'rib', 'cardio', 'breastbone', 'pneumonia', 'effusion', 'consolidation', 'cardiomegaly', 'atelectasis', 'pneumothorax'],
  brain: ['brain', 'skull', 'head', 'cranium', 'mri', 'ct', 'hemorrhage', 'infarct'],
  knee: ['knee', 'leg', 'thigh', 'tibia', 'femur', 'fibula', 'joint', 'fracture', 'osteophyte', 'meniscus'],
  abdomen: ['abdomen', 'stomach', 'torso', 'spine', 'kidney', 'renal', 'urinary', 'bladder', 'liver', 'spleen', 'nephrolithiasis', 'hydronephrosis', 'gallstone', 'hepatomegaly'],
  dental: ['tooth', 'teeth', 'jaw', 'mandible', 'maxilla', 'dental', 'caries', 'periodontitis'],
  hand: ['hand', 'finger', 'wrist', 'palm', 'fracture', 'carpal', 'metacarpal', 'phalanx'],
};
...
const loadVisionModels = async () => {
  ...
  if (!modelCache.chexnetModel) {
    try {
      modelCache.chexnetModel = await loadGraphModel(CHEXNET_MODEL_URL);
    } catch (e) {
      console.warn('CheXNet model failed to load:', e);
      throw new Error('CheXNet model is required and failed to load');
    }
  }

  if (!modelCache.radfmModel) {
    try {
      modelCache.radfmModel = await loadGraphModel(RADFM_MODEL_URL);
    } catch (e) {
      console.warn('RadFM model failed to load:', e);
    }
  }

  if (!modelCache.evaxModel) {
    try {
      modelCache.evaxModel = await loadGraphModel(EVAX_MODEL_URL);
    } catch (e) {
      console.warn('EVA-X model failed to load:', e);
    }
  }

  return { tf: modelCache.tf!, chexnetModel: modelCache.chexnetModel!, radfmModel: modelCache.radfmModel, evaxModel: modelCache.evaxModel };
};
...
const getImageInsights = (image: HTMLImageElement): ImageInsights => {
  const width = image.naturalWidth || image.width;
  const height = image.naturalHeight || image.height;
  const aspectRatio = width / Math.max(height, 1);
  let orientation: ImageInsights['orientation'] = 'square';
  if (aspectRatio > 1.15) orientation = 'landscape';
  else if (aspectRatio < 0.85) orientation = 'portrait';
  return { aspectRatio, orientation };
};

const mapPredictionToBodyPart = (predictions: MedicalPrediction): BodyPartKey | null => {
  for (const prediction of predictions) {
    const label = prediction.className.toLowerCase();
    const match = (Object.keys(BODY_PART_KEYWORDS) as BodyPartKey[]).find((bodyPart) =>
      BODY_PART_KEYWORDS[bodyPart].some((keyword) => label.includes(keyword)),
    );
...
const deriveBodyPartFromSignals = (
  predictions: MedicalPrediction,
  metrics: ImageMetrics,
  insights: ImageInsights,
): BodyPartKey => {
  const predictionMatch = mapPredictionToBodyPart(predictions);
  if (predictionMatch) return predictionMatch;

  const densityDelta = metrics.centralDensity - metrics.peripheralDensity;
  
  // Debug logging
  console.log('Detection debug:', {
    orientation: insights.orientation,
    aspectRatio: insights.aspectRatio,
    stdIntensity: metrics.stdIntensity,
    densityDelta,
    meanIntensity: metrics.meanIntensity,
  });
  
  // Chest signals (highest priority for portrait)
  if (insights.orientation === 'portrait') {
    console.log('Detected: chest (portrait priority)');
    return 'chest';
  }

  // Strong abdomen/kidney signals (must be landscape AND high variance)
  if (
    insights.orientation === 'landscape' &&
    metrics.stdIntensity > 35 &&
    densityDelta < -8
  ) {
    console.log('Detected: abdomen (strong signals)');
    return 'abdomen';
  }

  // Wide aspect ratio abdomen (must be landscape)
  if (insights.orientation === 'landscape' && insights.aspectRatio > 1.3 && densityDelta < -5) {
    console.log('Detected: abdomen (wide landscape)');
    return 'abdomen';
  }

  // Knee signals
  if (insights.orientation !== 'landscape' && metrics.stdIntensity > 30 && densityDelta < 8) {
    console.log('Detected: knee');
    return 'knee';
  }

  // Brain signals
  if (insights.aspectRatio >= 0.9 && insights.aspectRatio <= 1.1 && metrics.stdIntensity < 25) {
    console.log('Detected: brain');
    return 'brain';
  }

  // Dental signals
  if (metrics.meanIntensity > 150 && metrics.stdIntensity < 30) {
    console.log('Detected: dental');
    return 'dental';
  }

  // Hand signals (but check for abdomen/kidney first)
  if (insights.orientation !== 'landscape' && metrics.stdIntensity > 30) {
    // Check if this might be abdomen/kidney even in portrait
    if (densityDelta < -10 && metrics.meanIntensity < 140) {
      console.log('Detected: abdomen (portrait with kidney characteristics)');
      return 'abdomen';
    }
    console.log('Detected: hand');
    return 'hand';
  }

  // Default fallback based on density
  const fallback = densityDelta < -8 ? 'abdomen' : 'chest';
  console.log('Detected: fallback to', fallback);
  return fallback;
};
...
interface AnalysisResult {
  ...
  metrics: ImageMetrics;
  detectionReasoning: {
    predictionMatch: BodyPartKey | null;
    insights: ImageInsights;
    metrics: ImageMetrics;
    densityDelta: number;
  };
...
const analyzeImageAI = async () => {
    ...
    try {
      let detectedPart: BodyPartKey = 'chest';
      let confidence = 0.8;
      let predictionText = 'medical scan';
      let metrics: ImageMetrics = {
        meanIntensity: 128,
        stdIntensity: 30,
        centralDensity: 130,
        peripheralDensity: 125,
      };
      ...
      try {
        const { tf, chexnetModel, radfmModel, evaxModel } = await loadVisionModels();
        const imageElement = await loadImageElement(selectedImage);
        insights = getImageInsights(imageElement);
        metrics = await computeImageMetrics(tf, imageElement);

        // Parallel ensemble for robust X-ray detection
        const ensembleScores: { [key: string]: number } = {};

        // Run all models in parallel
        const modelPromises = [
          // CheXNet for chest X-rays
          (async () => {
            try {
              const tensor = tf.browser
                ...
                .div(255.0)
                .expandDims();
              const chexnetOutput = await chexnetModel.executeAsync(tensor) as tf.Tensor;
              const chexnetData = await chexnetOutput.data();
              const maxScore = Math.max(...Array.from(chexnetData.slice(0, 14)));
              tf.dispose([tensor, chexnetOutput]);
              ensembleScores.chest = maxScore;
            } catch (e) {
              console.warn('CheXNet inference failed:', e);
            }
          })(),
          // RadFM for general radiology
          (async () => {
            if (radfmModel) {
              try {
                const tensor = tf.browser
                  ...
                  .toFloat()
                  .expandDims();
                const radfmFeatures = await radfmModel.execute(tensor) as tf.Tensor;
                const radfmData = await radfmFeatures.data();
                const variance = Array.from(radfmData).reduce((sum, val, _, arr) => {
                  const mean = arr.reduce((a, b) => a + b, 0) / arr.length;
                  return sum + Math.pow(val - mean, 2);
                }, 0) / radfmData.length;
                ensembleScores.general = variance;
                tf.dispose([tensor, radfmFeatures]);
              } catch (e) {
                console.warn('RadFM inference failed:', e);
              }
            }
          })(),
          // EVA-X for musculoskeletal
          (async () => {
            if (evaxModel) {
              try {
                const tensor = tf.browser
                  ...
                  .toFloat()
                  .expandDims();
                const evaxFeatures = await evaxModel.execute(tensor) as tf.Tensor;
                const evaxData = await evaxFeatures.data();
                const avgFeature = Array.from(evaxData).reduce((a, b) => a + b, 0) / evaxData.length;
                if (insights.orientation === 'portrait' && metrics.stdIntensity > 30) {
                  ensembleScores.knee = avgFeature;
                  ensembleScores.hand = avgFeature * 0.8;
                }
                tf.dispose([tensor, evaxFeatures]);
              } catch (e) {
                console.warn('EVA-X inference failed:', e);
              }
            }
          })(),
        ];
        ...
        // Determine best body part from ensemble
        const bestScore = Object.entries(ensembleScores).sort((a, b) => b[1] - a[1])[0];
        if (bestScore) {
          switch (bestScore[0]) {
            case 'chest':
              detectedPart = 'chest';
            ...
            case 'hand':
              ...
              break;
            case 'general':
              detectedPart = deriveBodyPartFromSignals([], metrics, insights);
              confidence = 0.7;
              break;
          }
        } else {
        ...
        const predictions = [
          { className: detectedPart, probability: confidence },
          { className: 'medical_xray', probability: 0.9 },
        ];
        predictionText = predictions[0]?.className ?? 'medical scan';
      } catch (modelError) {
        console.warn('Model loading/inference failed, using heuristics:', modelError);
        ...
        detectedPart = deriveBodyPartFromSignals([], metrics, insights);
        confidence = 0.6;
        predictionText = 'heuristic_detection';
      }

      const diseaseSummary = buildDiseaseSummary(detectedPart, metrics, predictionText);
      const detectionReasoning = {
        predictionMatch: detectedPart,
        insights,
        metrics,
        densityDelta: metrics.centralDensity - metrics.peripheralDensity,
      };
    ...
    setStep('analysis');
      ...
      // Still show analysis with fallback data
      const fallbackAnalysis: AnalysisResult = {
        ...
        isSerious: false,
        confidence: 50,
        findings: ['Image analysis failed', 'Please try again or use a different image'],
        recommendations: ['Retry with a clearer image', 'Ensure proper image format'],
        followUp: 'Please retry the analysis',
        urgencyLevel: 'Low',
        technicalQuality: 'Analysis failed',
        diseaseSummary: {
          primaryCondition: 'Analysis Error',
          probability: 0,
          summary: 'The AI analysis encountered an error. Please try again.',
          supportingEvidence: ['Technical error during processing'],
          nextSteps: ['Retry with a different image', 'Check image quality'],
        },
        ...
        detectionReasoning: {
          ...
          insights: { aspectRatio: 1, orientation: 'square' },
          metrics: {
            meanIntensity: 0,
            stdIntensity: 0,
            centralDensity: 0,
                  ...
                  <div className="bg-white rounded-lg p-4 border border-gray-200">
                    ...
                    <ul className="space-y-2">
                      {analysis.findings.map((finding) => (
                        <li key={finding} className="flex items-start">
                ...
                <div className="bg-white rounded-lg p-4 border border-purple-200">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {analysis.diseaseSummary.supportingEvidence.map((item) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {analysis.diseaseSummary.nextSteps.map((item) => (
                          <li key={item} className="flex items-start text-sm text-gray-700">
