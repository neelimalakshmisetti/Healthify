...
const deriveBodyPartFromSignals = (
  ...
  // Strong abdomen/kidney signals
  if (
    insights.orientation === 'landscape' ||
    (metrics.stdIntensity > 35 && densityDelta < -8) ||
    (insights.aspectRatio > 1.3 && densityDelta < -5)
  ) {
    console.log('Detected: abdomen (strong signals)');
    return 'abdomen';
  }

  // Chest signals (more restrictive)
  if (insights.orientation === 'portrait' && densityDelta >= -5 && metrics.stdIntensity < 40) {
    console.log('Detected: chest (restrictive)');
    return 'chest';
  }
