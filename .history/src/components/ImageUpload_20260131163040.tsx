...
const modelCache: { tf?: TfModule; medicalModel?: MedicalModel } = {};

const MEDICAL_MODEL_URL = 'https://tfhub.dev/google/imagenet/mobilenet_v2_100_224/classification/5';

const BODY_PART_KEYWORDS: Record<BodyPartKey, string[]> = {
  chest: ['chest', 'lung', 'thorax', 'rib', 'cardio', 'breastbone'],
  brain: ['brain', 'skull', 'head', 'cranium'],
  knee: ['knee', 'leg', 'thigh', 'tibia', 'femur', 'fibula'],
  abdomen: ['abdomen', 'stomach', 'torso', 'spine', 'kidney', 'renal', 'urinary', 'bladder', 'liver', 'spleen'],
...
const loadVisionModels = async () => {
  ...
  return { tf: modelCache.tf!, medicalModel: modelCache.medicalModel! };
};
...
export const ImageUpload: React.FC<ImageUploadProps> = ({ onNavigate, onDiagnosis }) => {
  ...
  const analyzeImageAI = async () => {
    ...
    try {
      const { tf, medicalModel } = await loadVisionModels();
