...
export const ImageUpload: React.FC<ImageUploadProps> = ({ onNavigate, onDiagnosis }) => {
  ...
  const detectBodyPartFromImage = async (imageData: string): Promise<string> => {
    // In a real app, you would use a medical imaging model
    // For this example, we'll use MobileNet which is a general-purpose image classification model
    
    try {
      // Load the MobileNet model (in a real app, you'd use a medical imaging model)
      ...
      // Map general predictions to medical body parts
      const predictionText = predictions[0].className.toLowerCase();
      
      // Simple mapping of common objects to body parts
      // In a real app, you'd use a medical imaging model
      if (predictionText.includes('hand') || predictionText.includes('finger')) return 'hand';
      if (predictionText.includes('chest') || predictionText.includes('lung') || predictionText.includes('rib')) return 'chest';
      if (predictionText.includes('knee') || predictionText.includes('leg') || predictionText.includes('joint')) return 'knee';
      if (predictionText.includes('head') || predictionText.includes('brain') || predictionText.includes('skull')) return 'brain';
      if (predictionText.includes('tooth') || predictionText.includes('teeth') || predictionText.includes('dental')) return 'dental';
      if (predictionText.includes('abdomen') || predictionText.includes('stomach') || predictionText.includes('torso')) return 'abdomen';
      
      return 'chest'; // Default to chest X-ray
    } catch (error) {
      console.error('Error detecting body part:', error);
      return 'chest'; // Fallback to chest X-ray on error
    }
  };
  ...
  const analyzeImageAI = async () => {
    ...
    // Detect body part from image
    const detectedPart = await detectBodyPartFromImage(selectedImage);
    const selectedScan = scanTypes[detectedPart as keyof typeof scanTypes] || scanTypes.chest;
