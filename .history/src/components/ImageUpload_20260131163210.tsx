...
const modelCache: { tf?: TfModule; medicalModel?: MedicalModel; kidneyModel?: MedicalModel } = {};

const MEDICAL_MODEL_URL = 'https://tfhub.dev/tensorflow/efficientnet/b0/classification/1';
const KIDNEY_MODEL_URL = 'https://tfhub.dev/google/imagenet/mobilenet_v2_100_224/feature_vector/5';

const BODY_PART_KEYWORDS: Record<BodyPartKey, string[]> = {
  chest: ['chest', 'lung', 'thorax', 'rib', 'cardio', 'breastbone', 'pneumonia', 'effusion', 'consolidation', 'cardiomegaly'],
  brain: ['brain', 'skull', 'head', 'cranium'],
  knee: ['knee', 'leg', 'thigh', 'tibia', 'femur', 'fibula', 'joint', 'fracture'],
  abdomen: ['abdomen', 'stomach', 'torso', 'spine', 'kidney', 'renal', 'urinary', 'bladder', 'liver', 'spleen', 'nephrolithiasis', 'hydronephrosis'],
  dental: ['tooth', 'teeth', 'jaw', 'mandible', 'maxilla'],
  hand: ['hand', 'finger', 'wrist', 'palm', 'fracture'],
};
...
const loadVisionModels = async () => {
  ...
  if (!modelCache.medicalModel) {
    try {
      modelCache.medicalModel = await loadGraphModel(MEDICAL_MODEL_URL);
    } catch (e) {
      console.warn('Medical model failed to load, falling back to MobileNet:', e);
      const mobilenetModule = await import('@tensorflow-models/mobilenet');
      const fallback = await mobilenetModule.load();
      modelCache.medicalModel = fallback as any;
    }
  }

  if (!modelCache.kidneyModel) {
    try {
      modelCache.kidneyModel = await loadGraphModel(KIDNEY_MODEL_URL);
    } catch (e) {
      console.warn('Kidney model failed to load:', e);
    }
  }

  return { tf: modelCache.tf!, medicalModel: modelCache.medicalModel!, kidneyModel: modelCache.kidneyModel };
};
...
export const ImageUpload: React.FC<ImageUploadProps> = ({ onNavigate, onDiagnosis }) => {
  ...
  const analyzeImageAI = async () => {
    ...
    try {
      const { tf, medicalModel, kidneyModel } = await loadVisionModels();
      ...
      // Try kidney/abdomen model first for non-chest images
      if (kidneyModel && (insights.orientation === 'landscape' || metrics.stdIntensity > 25)) {
        const tensor = tf.browser
          .fromPixels(imageElement)
          .resizeNearestNeighbor([224, 224])
          .toFloat()
          .expandDims();
        const features = await kidneyModel.execute(tensor) as tf.Tensor;
        const data = await features.data();
        // Simple heuristic: high feature values suggest abdominal content
        const avgFeature = Array.from(data).reduce((a, b) => a + b, 0) / data.length;
        tf.dispose([tensor, features]);
        if (avgFeature > 0.5) {
          predictions = [{ className: 'abdomen_kidney', probability: avgFeature }];
        } else {
          predictions = [{ className: 'unknown', probability: 1 - avgFeature }];
        }
      } else {
        // Use general medical model
        if ('classify' in medicalModel) {
          predictions = await (medicalModel as any).classify(imageElement, 5);
        } else {
          const tensor = tf.browser
            .fromPixels(imageElement)
            .resizeNearestNeighbor([224, 224])
            .toFloat()
            .expandDims();
          const output = await medicalModel.executeAsync(tensor) as tf.Tensor;
          const data = await output.data();
          const softmax = Array.from(data).map((p, i) => ({
            className: `class_${i}`,
            probability: p,
          }));
          predictions = softmax.sort((a, b) => b.probability - a.probability).slice(0, 5);
