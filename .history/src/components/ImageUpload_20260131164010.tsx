...
const loadVisionModels = async () => {
  ...
  if (!modelCache.chexnetModel) {
    try {
      ...
      console.warn('CheXNet model failed to load, falling back to MobileNet:', e);
      const mobilenetModule = await import('@tensorflow-models/mobilenet');
      const fallback = await mobilenetModule.load();
      modelCache.chexnetModel = fallback as MedicalModel;
    }
  }
...
export const ImageUpload: React.FC<ImageUploadProps> = ({ onNavigate, onDiagnosis }) => {
  ...
  const analyzeImageAI = async () => {
    ...
    try {
      ...
      let predictions: MedicalPrediction;
      const insights = getImageInsights(imageElement);
      const metrics = await computeImageMetrics(tf, imageElement);

      // Multi-model ensemble for robust X-ray detection
      ...
      try {
        ...
        // Map CheXNet findings to body parts
        ensembleScores.chest = Math.max(...Array.from(chexnetData.slice(0, 14))); // First 14 are common findings
      ...
      if (radfmModel) {
        try {
          ...
          // Simple heuristic: high feature variance suggests medical content
      ...
      if (evaxModel) {
        try {
          ...
          // Heuristic for musculoskeletal content
      ...
      let detectedPart: BodyPartKey = 'chest'; // default
      if (bestScore) {
        switch (bestScore[0]) {
          case 'chest':
            detectedPart = 'chest';
          ...
          case 'hand':
            ...
            break;
          case 'general':
            // Use heuristics for general case
            detectedPart = deriveBodyPartFromSignals([], metrics, insights);
            break;
        }
      }

      // Generate predictions for UI
      predictions = [
        { className: detectedPart, probability: bestScore ? bestScore[1] : 0.8 },
        { className: 'medical_xray', probability: 0.9 },
      ];
      const detectedPart = deriveBodyPartFromSignals(predictions, metrics, insights);
      ...
      const detectionReasoning = {
        predictionMatch: mapPredictionToBodyPart(predictions),
        insights,
        metrics,
