...
const deriveBodyPartFromSignals = (
  ...
  // Strong abdomen/kidney signals (must be landscape OR high variance)
  if (
  ...
  // Wide aspect ratio abdomen
  if (insights.aspectRatio > 1.3 && densityDelta < -5) {
    console.log('Detected: abdomen (wide aspect)');
    return 'abdomen';
  }

  // Knee/leg signals (before hand)
  if (insights.orientation !== 'landscape' && metrics.stdIntensity > 30) {
    // Check for knee/leg characteristics
    if (densityDelta >= -5 && densityDelta < 8) {
      console.log('Detected: knee (leg characteristics)');
      return 'knee';
    }
